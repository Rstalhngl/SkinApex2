...
// ─── Listing Dialog ────────────────────────────────────────────────────────────
// Rendered as a sibling of <Sheet>, NOT inside SheetContent.
// This prevents Radix nested-modal pointer-events conflicts.
...
function ListingDialog({ item, refPrice, open, onClose, onList }: ListingDialogProps) {
  const [price, setPrice] = useState("")

  useEffect(() => {
    if (open && refPrice != null) setPrice(String(Math.round(refPrice)))
    if (!open) setPrice("")
  }, [open, refPrice])
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 rounded-lg border border-border bg-input p-3">
          <div
            className="relative flex h-14 w-16 shrink-0 items-center justify-center overflow-hidden rounded-md bg-card"
          >
            <div
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] font-semibold" style={{ color: item.rarityColor }}>
              {item.rarity}
            </p>
            {item.exterior && (
...
interface InventoryCardProps {
  ...
  /** Called when user clicks Listele — parent manages dialog state */
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-card p-3 transition-colors hover:border-[#243146]">
      ...
      <div className="mb-1.5 mt-0.5 flex items-center justify-between text-[9px]">
        <span className="font-semibold" style={{ color: item.rarityColor }}>
          {item.exterior || "—"}
        </span>
...
export function InventorySheet({ trigger }: { trigger?: React.ReactNode }) {
  ...
  // Listing dialog state — managed here so dialog renders OUTSIDE SheetContent
  ...
  useEffect(() => {
    if (open && steamProfile?.steamId && !fetchRef.current) {
      fetchRef.current = true
      fetchInventory()
    }
    if (!open) fetchRef.current = false
  ...
  const handleList = (priceTry: number) => {
  ...
  return (
    <>
          ...
          <SheetHeader className="border-b border-border px-5 py-4">
            <SheetTitle className="flex items-center gap-2 text-foreground">
              ...
              <Button
                ...
                onClick={() => { fetchRef.current = false; fetchInventory() }}
                disabled={loading}
              >
                <RefreshCw className={cn("h-3.5 w-3.5", loading && "animate-spin")} />
              </Button>
            </SheetTitle>
            <SheetDescription className="sr-only">{t("inventory.title")}</SheetDescription>
          </SheetHeader>
      ...
      {/* ListingDialog rendered OUTSIDE Sheet to avoid Radix nested-modal conflict */}
      <ListingDialog
        item={listingItem}
